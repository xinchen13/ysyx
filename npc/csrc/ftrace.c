...
static elfStFunc npc_elfstfunc;
...
void init_ftrace_stfunc(const char *elf_file) {
    ...
    for (int i = 0; i < ELF_ST_FUNC_NUM; i++) {
        for (int j = 0; j < ELF_ST_NAME_LEN; j++) {
            npc_elfstfunc.name[i][j] = '\0';
        }
        npc_elfstfunc.addr[i] = 0x00000000;
        npc_elfstfunc.size[i] = 0;
    }
    npc_elfstfunc.valid_func = 0;
    ...
    for (int i = 0; i < sh_table[symtab_index].sh_size / sizeof(Elf32_Sym); i++) {
        ...
        if (ELF32_ST_TYPE(symbol.st_info) == STT_FUNC) {
            strcpy(npc_elfstfunc.name[npc_elfstfunc.valid_func], symbol_name);
            npc_elfstfunc.addr[npc_elfstfunc.valid_func] = symbol.st_value;
            npc_elfstfunc.size[npc_elfstfunc.valid_func] = symbol.st_size;
            npc_elfstfunc.valid_func++;
        }
    }
...
void display_elfstfunc() {
    for (int i = 0; i < npc_elfstfunc.valid_func; i++) {
        printf("func %d: name=%s, addr=%08x, size=%d\n", i, npc_elfstfunc.name[i], npc_elfstfunc.addr[i], npc_elfstfunc.size[i]);
    }
}

void ftrace_call(word_t curr_addr, word_t j_addr, int depth) {
    for (int i = 0; i < npc_elfstfunc.valid_func; i++) {
        if ((j_addr >= npc_elfstfunc.addr[i]) && (j_addr < npc_elfstfunc.addr[i] + npc_elfstfunc.size[i])) {
            printf(FMT_PADDR ":%*scall [%s@" FMT_PADDR "]\n", curr_addr, depth, " ", npc_elfstfunc.name[i], j_addr);
        }
    }
}
...
void ftrace_retn(word_t curr_addr, int depth) {
    for (int i = 0; i < npc_elfstfunc.valid_func; i++) {
        if ((curr_addr >= npc_elfstfunc.addr[i]) && (curr_addr < npc_elfstfunc.addr[i] + npc_elfstfunc.size[i])) {
            printf(FMT_PADDR ":%*sretn [%s]\n", curr_addr, depth, " ", npc_elfstfunc.name[i]);
        }
    }
}