...
用RTL为IFU编写一个SRAM模块, 地址位宽为32bit, 数据位宽为32bit. 收到读请求后, SRAM模块通过DPI-C调用`dpic_pmem_read()`读出数据, 并延迟1周期返回读出的数据, 来实现一个更真实的存储器

由于这个SRAM需要经过1周期才能拿到读出的数据, 这时候NPC已经不是一个严格意义上的单周期处理器了, 而是一个简单的多周期处理器:
- 在第1个周期, IFU发出取指请求
