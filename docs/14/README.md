...
### 实现AXI-Lite接口的UART功能
- 编写一个AXI-Lite接口的slave模块, 当往这个设备寄存器发送写请求时, 通过`uart_out(int wdata)`输出. 为了方便测试, 地址设置成与之前仿真环境中串口的地址相同
- 编写一个Xbar模块, 根据地址来转发通信请求, 将这个具备UART功能的模块接入系统中

###  实现AXI-Lite接口的CLINT
CLINT(Core Local INTerrupt controller)是RISC-V系统中较通用的中断控制器, 是一个用于维护时钟中断和软件中断的模块. 不过目前我们的系统还不需要中断功能, 因此先考虑时钟相关的功能: 

- CLINT包含一个只读的设备寄存器mtime, 它会以一定的速率增长, 最简单的实现是每周期加1. 同样地, 为了方便测试, 其地址可以设置成与之前仿真环境中时钟的地址相同
- mtime的增加还不能直接反映时间的流逝, 它们之间相差一个系数, 需要由软件读出后进行处理. 在真实的处理器芯片中, 一般这个系数等于CLINT模块中的时钟频率, 从而可以让软件测量出真实的时间. 不过仿真环境中没有主频的概念, 如果这个系数等于仿真速率, 我们就可以在仿真环境中通过mtime的流逝计算出真实时间的流逝. 具体地, 需要修改IOE的相关代码, 让`AM_TIMER_UPTIME`返回的时间接近真实时间
- 为了避免在实际使用中发生溢出, mtime是64位的. 但目前NPC是32位的, 如果只读出mtime的低32位, 在一段时间之后, mtime将会发生溢出(尽管不太容易在仿真环境中运行到mtime溢出的时刻, 但如果NPC将来运行在500MHz的频率下, 将大概率会发生溢出), 从而使系统的时间功能发生错误. 因此, 运行在32位NPC上的软件需要依次读出mtime的低32位和高32位, 将其组合成一个64位的值, 供上层应用使用