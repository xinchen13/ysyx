...
void Database_find(struct Connection *conn, char *pattern, char *find_key)
{
    struct Database *db = conn->db;
    ...
    for(int i = 0; i < MAX_ROWS; i++){
        ...
        if (strcmp(pattern, "name") == 0){
            ...
            die("Input the f**king right attribute!",conn);
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    switch(action) {
        ...
        case 'f':
            if(argc != 5) die("Need patern, key to set", conn);
