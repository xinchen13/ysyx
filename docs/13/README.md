...
- 观察到两点: 最终仿真环境到达时间上限后退出时会打印`msh />`; 对于yield-os来说有同样的问题, A与B在程序结束后才输出
- 原因暂不知

RISC-V完整的异常处理机制远比现在实现的复杂, 而商业化的RISC-V处理器必须准确无误地实现手册中描述的每一个细节, 不管客户是否会用到. 实现所有CSR并细扣其中的每一个比特, 其实是一项偏工程属性的工作, 需要工程团队投入很多精力. 随着将来在A阶段运行更多的程序, 也会逐渐添加各种CSR及其核心功能
